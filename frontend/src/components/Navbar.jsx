        ...
        <ul className="hidden lg:flex items-center gap-1">
          {NAV_LINKS.map(({ label, href, dropdown, badge }) => (
            <li
              key={label}
              className="relative"
              onMouseEnter={() => dropdown && openDropdownMenu()}
              onMouseLeave={() => dropdown && scheduleCloseDropdown()}
            >
              {dropdown ?
                <button
              ...
              {dropdown && openDropdown && (
                <div
                  className="dropdown-panel absolute top-full left-0 mt-2 w-[280px] max-h-[320px] overflow-y-auto bg-white/95 shadow-[0_20px_45px_-20px_rgba(22,24,27,0.45)] rounded-2xl border border-gray-200 py-2 z-50 backdrop-blur"
                  onMouseEnter={() => dropdown && openDropdownMenu()}
                  onMouseLeave={() => dropdown && scheduleCloseDropdown()}
                >
                  {dropdown.map((item) => (
              ...
              return (
                <div key={label} className="flex flex-col gap-2">
                  <button
                    ...
                    className="px-3 py-2 rounded-full text-sm text-left text-gray-700 hover:bg-red-50/70 hover:text-[#A11717] transition-all duration-300 flex items-center justify-between"
                  >
                    <span>{label}</span>
                    <ChevronDown
                  ...
                  {openDropdown && (
                    <div className="ml-3 flex flex-col gap-1 border-l border-red-100 pl-3">
                      {dropdown.map((item) => (
                        <NavLink
                          key={item.label}
                          to={item.href}
                          ...
                          className="text-sm text-gray-600 hover:text-[#A11717] transition-all duration-300 no-underline py-1"
                        >
                          {item.label}
                        </NavLink>
                      ))}
